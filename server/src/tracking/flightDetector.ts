...
export const DETECTOR_DEFAULTS = {
  ...
  airGsKt: 55, // at/above this => flying (≈ rotation speed for the fleet)
...
export interface DetectorEvents {
  onFlightStarted?: (flightId: number, aircraftId: number, callsign: string | null) => void;
  onFlightEnded?: (flightId: number, aircraftId: number) => void;
}

export class FlightDetector {
  private states = new Map<number, AircraftState>();
  private airfields: AirfieldRow[] = [];
  ...
  reloadAirfields(): void {
    this.airfields = this.db.prepare('SELECT * FROM airfields').all() as AirfieldRow[];
  }
  ...
  private stateFor(aircraftId: number): AircraftState {
    let s = this.states.get(aircraftId);
    if (!s) {
      s = { state: 'ground', flightId: null, last: null, lastGroundFix: null };
      this.states.set(aircraftId, s);
    }
    return s;
  }

  nearestAirfield(lat: number, lon: number): { af: AirfieldRow; distNm: number } | null {
    let best: { af: AirfieldRow; distNm: number } | null = null;
    for (const af of this.airfields) {
  ...
  classify(p: NormPosition): Classification {
    if (p.onGround) return 'ground';
    const near = this.nearestAirfield(p.lat, p.lon);
  ...
  private looksLikeLanding(p: NormPosition): { landing: boolean; airfieldId: number | null } {
    const near = this.nearestAirfield(p.lat, p.lon);
    ...
    if (low && (inside || ((descending || agl < 700) && toward))) {
      return { landing: true, airfieldId: near.af.id };
    }
    return { landing: false, airfieldId: null };
  }

  private airfieldIdNear(lat: number, lon: number): number | null {
    const near = this.nearestAirfield(lat, lon);
    return near && near.distNm <= this.cfg.airfieldMatchNm ? near.af.id : null;
  }

  private startFlight(aircraftId: number, p: NormPosition, s: AircraftState): void {
    ...
    const recent = this.db
      ...
      .get(aircraftId) as { id: number; ended_at: number; end_confidence: string } | undefined;
    ...
    const originId = this.airfieldIdNear(originFix.lat, originFix.lon);
    const originCode = originId
      ? (this.airfields.find((a) => a.id === originId)?.code ?? null)
      : null;
    const res = this.db
      .prepare(
      ...
      .run(
        ...
        originId,
        originCode,
        originCode ? 'detected' : null,
        Date.now()
      );
    s.flightId = Number(res.lastInsertRowid);
    s.state = 'airborne';
    this.events.onFlightStarted?.(s.flightId, aircraftId, p.callsign);
  }
  ...
  private endFlight(
    ...
    const destCode = destAirfieldId
      ? (this.airfields.find((a) => a.id === destAirfieldId)?.code ?? null)
      : null;
    this.db
      .prepare(
    ...
    this.events.onFlightEnded?.(s.flightId, aircraftId);
    s.flightId = null;
    s.state = 'ground';
  }
  ...
  // A reappearance is a plausible continuation if the aircraft could have
  // covered the distance in the elapsed time at roughly its last speed.
  ...
  onPosition(aircraftId: number, p: NormPosition): number | null {
    const s = this.stateFor(aircraftId);
    ...
    if (s.state === 'ground') {
      if (cls === 'air') {
        this.startFlight(aircraftId, p, s);
      ...
      if (brokenContinuation) {
        const landing = s.last ? this.looksLikeLanding(s.last) : { landing: false, airfieldId: null };
        this.endFlight(
          s,
          aircraftId,
          s.last!.ts,
          landing.landing ? 'assumed' : 'lost',
          landing.airfieldId
        );
        ...
        this.endFlight(s, aircraftId, p.ts, 'confirmed', this.airfieldIdNear(p.lat, p.lon));
        s.lastGroundFix = { lat: p.lat, lon: p.lon };
        s.last = p;
        return flightId;
      } else {
  ...
  tick(now: number): void {
    for (const [aircraftId, s] of this.states) {
      ...
      if (s.state === 'airborne' && silenceSec > this.cfg.silenceClassifySec) {
        const landing = this.looksLikeLanding(s.last);
        if (landing.landing) {
          this.endFlight(s, aircraftId, s.last.ts, 'assumed', landing.airfieldId);
        } else {
          s.state = 'lost';
        }
      } else if (s.state === 'lost' && silenceSec > this.cfg.lostTimeoutSec) {
        this.endFlight(s, aircraftId, s.last.ts, 'lost', null);
      }
    }
  }
